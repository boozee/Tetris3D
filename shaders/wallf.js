...
{
	//lambert
	vec3 lambert = vec3(0.0, 0.0, 0.0);
	for( int i = 0; i < 2; i++)
	{
		vec3 color = lightPower[i] / ( 4.0 * PI * pow( length(lightVector[i]),2.0) );
		vec3 lVector = normalize( lightVector[i] );
		float dotProduct = dot( transformedNormal, lVector );
		float pointLightWeighting = max( dotProduct, 0.0 );
		lambert += color * pointLightWeighting * rho/PI;
	}
	//microfacet
	...
	gl_FragColor = vec4( ambient, 1.0 ) + (vec4( color, 1.0 ) + vec4(lambert, 1.0 )) * (1.0 - wthrp);
}